...
function init(app) {
  ...
  app.addSingleton("knex", (config, app) => {
    ...
    if (!isSqlite(config.client)) {
      if (!connection.connectString) {
        assert(
          connection.host && connection.user && connection.database,
          `[egg-knex] ${config.client} 'host: ${connection.host}', 'user: ${
            connection.user
          }', 'database: ${connection.database}' are required on config`
        );
        app.knexLogger.info(
          "[egg-knex] %s connecting %s@%s:%s/%s",
          config.client,
          connection.user,
          connection.host,
        ...
        assert(
          connection.user && connection.connectString,
          `[egg-knex] ${config.client} 'user: ${
            connection.user
          }', 'connectString: ${
...
function createInstance(app, config, callback) {
  ...
  co(function* isReady() {
    ...
    .then(num => {
      app.knexLogger.info(
        `[egg-knex] ${config.client} ${clientDesc(
          config.connection
        )} status OK, got ${num} tables`
      );
...
function injectInstrument(app, knex) {
  ...
  ["query", "stream"].forEach(method => {
    ...
    client[method] = function(connection, obj) {
      ...
      const ins = app.instrument(
        client.dialect,
        this._formatQuery(obj.sql, obj.bindings)
      );
  ...
  proto.transaction = function(...args) {
    ...
    return new Promise(resolve => {
      ...
      transaction.rollbackTo = function(conn, error) {
        return this.query(
          conn,
          `ROLLBACK TO SAVEPOINT ${this.txid}`,
          error ? 2 : 1,
          error
        )
          .timeout(5000)
