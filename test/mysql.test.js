...
describe('test/mysql.test.js', () => {
  ...
  afterEach(function* () {
    // 清空测试数据
    yield app.mysql.del().from('npm_auth').where('user_id', 'like', `egg-${uid}%`);
  });

  after(done => {
    app.mysql.destroy().then(done);
  });
  ...
  it('should update successfully', function* () {
    const user = yield app.mysql
      .select('*')
      .from('npm_auth')
      .orderBy('id', 'desc')
      .limit(1);

    const result = yield app.mysql
